...
- **The Development menu is for developers who know both this codebase and the BL-18C beamline well — not for general beamline users.** Its tools skip the safety/UX guardrails the rest of the app has (e.g. `Pm16cConsoleWindow` sends raw ASCII commands straight to the PM16C, bypassing `MOVE_CONSTRAINTS` and speed/move limits entirely). Don't soften that raw access with extra validation/confirmation "for safety" — the tool's value is direct, unmediated hardware access for someone who already understands the protocol and the physical consequences; adding guardrails would defeat its purpose (agreed with the user, 2026-07-14).
