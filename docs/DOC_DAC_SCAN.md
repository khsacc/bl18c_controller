...
ガスケットの穴径とビーム径の兼ね合いによっては、台形状の函数の方がデータをよくフィットできる場合があります。ただし、透過X線で見た穴の形状が十分対称で綺麗であれば、Gaussian でフィット自体が多少悪かったとしても、中心位置の計算にはほとんど影響しません。以下、実機を用いたテスト測定データを、二つの関数でフィットした結果を示します。

![](./assets/dac_scan/function_comparison.jpg)
