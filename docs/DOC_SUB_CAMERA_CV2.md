...
### 3. Tenegrad と Laplacian の比較

> [! Important]
> GUI ではオートフォーカスに用いる手法を選択できる余地を残していますが、常に Tenegrad を利用することを推奨します。

- 実機を用いたテストにおいても、基本的には Tenegrad で全てうまくいき、総じて Tenegrad のほうがはっきりとコントラストがつきます。おそらく、Tenegrad が一次微分を利用するのに対して、Laplacianは２次微分を利用するため、少しボケている部分があるとそれに引きずられてシャープネスの評価が落ち込むことが一因と考えられ、DAC試料のようにガスケットと試料で異なるピントの合い方をするケースではあまりうまくいかないのかもしれません。
- Tenegrad は画素ごとの強度の一次微分を利用するため、Laplacian（二次微分）よりノイズに強く、特に試料の振動する低温実験ではこの効果は顕著なものとなります。
