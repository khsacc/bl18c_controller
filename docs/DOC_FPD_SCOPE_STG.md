...
これらの安全対策のパラメータは、全てのアプリに対して共通に例外なく適用するものであるから、各アプリ内には定義せず、 [utils/stage/control_stage.py](../../utils/stage/control_stage.py) にまとめて定義されている。2026年7月時点での実装内容は、以下のとおりであり、他のステージに対してもリミッターをかけたい場合には、同様の文法で記述すれば、リミッターを追加することができる。
