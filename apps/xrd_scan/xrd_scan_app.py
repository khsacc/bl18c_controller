...
class XrdScanWindow(QMainWindow):
    ...
    def _build_param_panel(self) -> QWidget:
        ...
        self._scan_size_ch4_spin.setDecimals(0)
        ...
        self._scan_size_ch5_spin.setDecimals(0)
        ...
        x_range_row.addWidget(QLabel(tr("Ch4 fit range (pulse):")))
        ...
        y_range_row.addWidget(QLabel(tr("Ch5 fit range (pulse):")))
    ...
    def _run_fit(self) -> None:
        ...
        xp_fine = np.linspace(xp_f[0], xp_f[-1], 300) if xp_f.size else xp
        yp_fine = np.linspace(yp_f[0], yp_f[-1], 300) if yp_f.size else yp
        ...
        if model == "Gaussian":
            ...
            try:
                cx, wx, popt_x, flipped_x = _fit_aperture_1d(xp_f, x_profile_f)
                x_center_rel = cx
                yy = _aperture_model(xp_fine, *popt_x)
                if flipped_x:
                    yy = float(np.nanmax(x_profile_f)) - yy
                ...
                self._fit_x_label.setText(tr("Ch{ch}:  fit failed", ch=4))
            try:
                cy, wy, popt_y, flipped_y = _fit_aperture_1d(yp_f, y_profile_f)
                y_center_rel = cy
                yy = _aperture_model(yp_fine, *popt_y)
                if flipped_y:
                    yy = float(np.nanmax(y_profile_f)) - yy
