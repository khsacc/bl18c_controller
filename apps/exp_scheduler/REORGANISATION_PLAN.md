...
現在の実装では、例えば次の挙動が発生する。

- `wait()` が AST validation を通り、`WaitAction(duration_s=0.0)` になる。
- `log_message()` が空メッセージとして通る。
- `follow_sample_position()` が `duration_s=0.0` になる。
- `wait(foo=123)` の未知の keyword が拒否されず、`foo` が捨てられる。
- `log_message("hello")` の positional argument が無視される。
- `set_pressure(pressure=1, unit="MPa")` が、`rate=None`,
  `rate_unit=None` の `SetPressureAction` になる。
- `take_xrd(oscillate=True, ...)` の振動引数が parser で Action に渡らず消失する。
- `normal_stop()` は API、parser、Action に存在するが、`ALLOWED_FUNCTIONS` にないため
  DSL では拒否される。

一部は PreValidator が最終的に検出する。しかし、0 秒の wait や parser が捨てた
引数のように、後段では元の DSL の意図を復元できず、意味が静かに変わるものもある。
...
├── dsl/
│   ├── commands.py             # CommandSpec と Action factory の正本
...
既存 DSL との互換性のため、初期移行では signature が許す positional argument は受け入れる。
新しい `to_dsl()` 出力と LLM example は引き続き明示的 keyword を使う。全 command を
keyword-only に変更する場合は別途 breaking change として判断する。
...
1. 全 DSL command について次の表をテストデータとして作る。
   - 最小有効呼び出し
   - 必須引数
   - optional 引数と default
   - 対応 Action type
   - loop variable を許す引数
2. `api.py`、`ALLOWED_FUNCTIONS`、`SequenceBuilder._BUILDERS`、Action type の集合差を
   自動検査するテストを追加する。
3. 現在確認済みの silent acceptance / argument loss を regression test にする。
4. `Action.to_dsl() -> compile -> Action` の round-trip test を command ごとに追加する。
5. Visual / JSON から直接不正な Action が入るケースを PreValidator test に追加する。
6. 実装に入る前に、現在の文書とコードで食い違う次の contract を決定し、この文書または
   `SPEC.md` に記録する。
   - `Assign` / `If` をこの再編で実装するか、実装まで DSL から拒否するか。
   - `TakeXrdAction.to_dsl()` が出力する per-step override を DSL の公開引数にするか、
     `to_dsl()` の出力対象から外すか。
   - positional argument は既存 signature どおり受け入れるか、将来 keyword-only にするか。
...
現在、LLM 経路だけ normalizer を使い、Script Editor は直接 `ast.parse()` している。この Phase
で両者を同じ normalizer 経路に統一する。正規化後の text と AST は同じ compile 結果から
使用し、再度別々に parse しない。
...
- `wait()`, `log_message()`, `follow_sample_position()` を compile error にする。
- `wait(foo=123)` を unknown argument error にする。
- positional argument は signature に従って処理し、無視しない。
- `set_pressure(..., rate/rate_unit省略)` を Action 生成前に拒否する。
...
1. `dsl/commands.py` に `CommandSpec`, `ArgumentRule`, registry を追加する。
2. 全 command の signature、metadata、factory を移す。
...
装置通信を含む実行前検証という価値を維持しつつ、約 1900 行の PreValidator を装置別に
分離し、一回の validation 内で一貫した装置状態を参照する。
...
6. `validator/checks/` を装置単位で追加し、現在の `_check_*` を一つずつ移す。
7. `PreValidator.validate()` は次だけを担当する facade に縮小する。
   - trace 作成
   - snapshot 収集
   - checker 実行
   - Diagnostic 集約
   - validation log 出力
8. 一つの checker / 通信が例外を出しても、可能な他のチェックは継続する。
   ただし必要情報が読めなかった安全判定は fail-open にしない。
9. `validator/VALIDATOR.md` の各項目に対応する checker test が存在することを一覧で確認する。
...
3. 現在の `_validated_positions` を StageSnapshot / certificate へ移行する。
4. Run 押下時に sequence / settings fingerprint を照合する。
5. fingerprint が一致しても live preflight を再実行する。
6. Stage position、PACE source pressure、接続状態などが変化して新しい error が出た場合は Run を
   拒否する。
7. warning の内容が変わった場合は、最新 warning に対してのみ続行確認を出す。
8. revalidation 成功後にのみ `SequenceRunner` を生成・start する。

#### 完了条件

- Validate 後に Sequence または settings を変更すると Run できない。
- ステージ位置が変化した場合、既存と同等以上の安全性で Run が拒否される。
...
- Run 押下時に live preflight が再実行される。
...
8. Stage current positions、PACE source pressure 等が Validate と Run 直前に確認される。
...
12. narrow test、scheduler test、simulation test が通り、残る実機依存リスクが文書化されている。
