...
- `apps/ui_stage_controller/__localdata/stage_settings.json`（キー名の確認）
...
- `stage_settings.json` のキー名は `stage_controller.py` の実装と合わせる（`det_in`, `det_out`, `ch8_in`, `ch8_out`）
...
- Mode 1 では `ForLoopAction` は追加できない（DSL 専用）。`ForLoopAction` が選択肢に出ないようにする
...
- `apps/ui_stage_controller/stage_controller.py`（`shortcut_1`, `shortcut_2` メソッドで移動順序を確認）
- `apps/ui_stage_controller/__localdata/stage_settings.json`（キー名の確認）
...
3. 順序：Ch8 OUT → 完了後 Ch9 IN（`stage_controller.py` の `shortcut_1` と同じ順序）
